...
- **Yamoah Attafuah:** Managed DevOps operations and architecture. Setup the multi-node DigitalOcean infrastructure and orchestrated the Docker Swarm deployment. Developed an automated GitHub Actions CI/CD pipeline to build and deploy environment-agnostic Docker images directly to production. Implemented Docker Secrets for database credentials to harden backend security, and deployed a custom centralized observability stack to monitor all container logs in real-time. Co-produced the final project video demonstration, highlighting the infrastructure, deployments, and observability stack. *(See Git history in the repository root, `.github/`, and additional integration commits in `frontend/` and `backend/`.)*
- **Samson Ajadalu:** Developed the robotics telemetry bridge. Wrote the Python nodes in ROS 2 to extract data from Gazebo/TurtleBot3 and stream it to the cloud backend. Handled full-stack integration by debugging WebSocket connections and fixing CORS policy errors between the frontend and API. Also authored the project's open-source documentation, including the main README and contribution guidelines. Co-produced the final project video demonstration, showcasing the live dashboard features, robot simulation and telemetry pipeline. *(See Git history under `src/robot_bridge/`, with related integration commits in `frontend/`, `backend/`, and the repo root.)*
